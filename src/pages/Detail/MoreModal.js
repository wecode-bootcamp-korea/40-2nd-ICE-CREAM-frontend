...
const BTN_LIST = [
  { id: 1, list: '체결 거래 ', data: 'soldList' },
  { id: 2, list: '판매 입찰', data: 'sellList' },
  { id: 3, list: '구매 입찰', data: 'buyList' },
];
...
const MoreModal = ({ setIsMoreClicked }) => {
  ...
  useEffect(() => {
    ...
    const datanum = dataList.data;
    const observer = new IntersectionObserver(e => {
      if (e[0].isIntersecting) {
        fetch('/data/chartTotalData.json', {
          method: 'GET',
        })
          .then(response => response.json())
          .then(result => {
            setTableData(result[0]);
            console.log('cons : ', ...result[0][datanum]);
            setLoadData(prev => [...prev, ...result[0][datanum]]);
          });
      }
    });
    if (ref.current) {
