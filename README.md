...
## 카카오 로그인 (문다연)
- OAuth 2.0 기반의 카카오 로그인 구현
- 로그인 버튼 클릭 시 카카오 동의화면을 호출, 사용자가 동의하게 되면 사용자에 대한 인가코드를 발급해 redirect uri로 전달
- 백엔드에서 토큰을 받아 사이트 자체 토큰을 생성하면 그 토큰을 가져와 로그인이 되도록 구현

## 스타일 페이지 (문다연)
- 인기순 / 최신순 필터 기능을 사용해 인기순 클릭 시 좋아요 갯수가 많은 순으로 나타나도록 구현
- 좋아요 버튼 구현으로 사용자간 소통할 수 있도록 구현
- Style Page에서 이미지 클릭 시 Style Detail Page로 이동, Style Detail Page에서는 해당하는 제품을 볼 수 있도록 구현
